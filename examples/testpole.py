...
def evaluate_individual(phenotype, test = False, **kwargs):
    if len(phenotype) == 0:
        return 1e9
    try:
...
if __name__ == "__main__":
    ...
    #edw.run()
    bestfh = open(os.getenv('SGE_JOB')+'_best_'+os.getenv('SGE_TASK_ID')+'.save','r')
    edw.best = edw.agentclass(gcode = pickle.loads(bestfh),
                               parent = None,
                               problem = p)
    ...
    results = []
    for i in inputs:
        fit = evaluate_individual(edw.best.phenotype, test=True)
        if fit < 0.000000001:
            results.append(0)
        else:
            results.append(1)
